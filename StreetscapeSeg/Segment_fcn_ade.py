...
model = gluoncv.model_zoo.get_model('fcn_resnet101_ade',ctx=ctx,pretrained=True )

filelist = os.listdir(file_path)
col=['id','lng','lat','heading']
...
for i in filelist: # 循环遍历所有的图片进行语义分割，并将结果存如pd.DataFrame
    ...
    if i not in filelist_out:
        try:
            ...
            # 8_112.542550679,26.9030039551_201609_90.949.jpg
            lng = 0
            lat = 0
            heading=0
            ...
            plt.close('all')
            # 输出结果的路径 csv文件
            df.to_csv("./img_seg_fcn_ade20k.csv")  # 将结果保存到csv
        except:
            pass
    else:
        print(i,'已经存在！')
