...
model = gluoncv.model_zoo.get_model('deeplab_resnet101_citys',ctx=ctx,pretrained=True )

filelist = os.listdir(file_path)

df = pd.DataFrame(columns=['id','lng','lat','pixels','road','sidewalk','building','wall','fence',
...
for i in filelist: # 循环遍历所有的图片进行语义分割，并将结果存如pd.DataFrame
    ...
    if i not in filelist_out:
        ...
        plt.close('all')
        # 输出结果的路径 csv文件
        df.to_csv("./img_seg_deeplab_cityscape.csv")  # 将结果保存到csv
    else:
        print(i,'已经存在！')
